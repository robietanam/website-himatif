...
@section('content') {{-- content --}}
    <div class="row gutters-xs align-items-center justify-content-end my-4">
        ...
        <div class="col col-md-auto">
            <a
                href="{{ url('dashboard/admin/page-contents') }}"
    ...
    <div class="row gutters-xs">
        ...
        <div class="col-lg">
            <div class="card">
                <div class="card-body">
                    <form action="{{ url("dashboard/admin/page-contents/$content->id") }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        @method('PUT')
                        @php
                                ...
                                {{-- {{ dd($contentData) }} --}}
                                    ...
                                    @if (file_exists(storage_path('app/public/'.$contentData->content)))
                                        <div class="img-fit img-fit-contain w-100 h-16rem bg-whitesmoke my-3">
                                            <img id="img-content-{{ $i }}" src="{{ asset('storage/' . $contentData->content) }}" alt="Post">
                                        </div>
                                    @else
                                        <div class="font-weight-600">Preview Upload</div>
                                        <div class="img-fit img-fit-contain w-100 h-16rem bg-whitesmoke mb-3">
                                            <img id="img-content-{{ $i }}" src="{{ asset('img/placeholder/product-image-default.svg') }}" alt="Post Placeholder">
                                        </div>
                                    @endif
                                ...
                                @case('mediumtext')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea
                                            ...
                                            cols="30"
                                            rows="8">{{ old($contentData->name) ? old($contentData->name) : $contentData->content }}</textarea>
