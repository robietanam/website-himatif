@if ($paginator->hasPages())
    <div class="ui flex list-reset pl-0 rounded-md menu" role="navigation">
        {{-- Previous Page Link --}}
        @if ($paginator->onFirstPage())
            <a class="icon item opacity-75" aria-disabled="true" aria-label="@lang('pagination.previous')"> <i
        ...
        @foreach ($elements as $element)
            ...
            @if (is_string($element))
                <a class="icon item opacity-75" aria-disabled="true">{{ $element }}</a>
            @endif
        ...
        @else
            <a class="icon item opacity-75" aria-disabled="true" aria-label="@lang('pagination.next')"> <i
