...
@section('breadcrumb') {{-- breadcrumb --}}
    <div class="inline-block px-2 py-2 text-gray-700 active"><a href="{{ route('dashboard.admin.prokers.index') }}">Proker</a>
    </div>
    <div class="inline-block px-2 py-2 text-gray-700">Atur Data</div>
                        ...
                        <div
                            ...
                            <div class="flex-auto p-6">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="description" id="summernote-editor" cols="30" rows="8">{{ old('description') ? old('description') : $proker->description }}</textarea>
                                    @error('description')
                                ...
                                @component('dashboard._components._form-group.input-radio')
                                    ...
                                    @slot('inputDatas', [
                                        '<i class="fas fa-check-square text-green-500 mr-1"></i> Aktif' => 1,
                                    ...
                                    @component('dashboard._components._form-group.input-radio')
                                        ...
                                        @component('dashboard._components._form-group.input')
                                            @slot('inputLabel', 'Nomor WA CP')
                                            @slot('inputName', 'link_contact_person')
                                            @slot('inputHelp', 'Nomor WA CP PJ Humas, Contoh : 0895326363837')
                                            @slot('inputId', 'input-link_contact_person')
                                            @slot('inputValue', $proker->link_contact_person)
                                        @endcomponent

                                        {{-- timeline --}}
                                        @component('dashboard._components._form-group.input-multiple')
                                            @slot('inputLabel', 'Timeline Proker')
                                            @slot('inputName', 'timeline')
                                            @slot('inputHelp', 'Wajib diisi bila timeline diaktifkan')
                                            @slot('inputId', 'input-timeline')
                                            @slot('inputValue', $proker->timeline)
                                        @endcomponent

                                        {{-- input : is_timeline_open --}}
                                        @component('dashboard._components._form-group.input-radio')
                                            @slot('inputLabel', 'Tampilkan Timeline ?')
                                            @slot('inputName', 'is_timeline_open')
                                            @slot('inputId', 'input-is_timeline_open')
                                            @slot('inputValue', old('is_timeline_open') ?? ($proker->is_timeline_open ? '1' : '0'))
                                            @slot('inputDatas', [
                                                '<i class="fas fa-share-square text-blue-600 mr-1"></i> Tampil' => 1,
                                                '<i class="fas fa-minus-square mr-1"></i> Sembuyikan' => 0,
                                                ])
                                            @endcomponent


                                            {{-- dokumentasi --}}
                                            @component('dashboard._components._form-group.input-multiple-image')
                                                @slot('inputLabel', 'Dokumentasi Proker')
                                                @slot('inputName', 'dokumentasi[]')
                                                @slot('inputPreviewIdentity', 'img-logo')
                                                @slot('inputHelp', 'Wajib diisi bila Dokumentasi diaktifkan')
                                                @slot('inputId', 'input-dokumentasi')
                                                @slot('inputValue', $proker->dokumentasi)
                                            @endcomponent

                                            {{-- input : is_dokumentasi_open --}}
                                            @component('dashboard._components._form-group.input-radio')
                                                @slot('inputLabel', 'Tampilkan Galeri ?')
                                                @slot('inputName', 'is_dokumentasi_open')
                                                @slot('inputId', 'input-is_dokumentasi_open')
                                                @slot('inputValue', old('is_dokumentasi_open') ?? ($proker->is_dokumentasi_open ? '1' :
                                                    '0'))
                                                    @slot('inputDatas', [
                                                        '<i class="fas fa-share-square text-blue-600 mr-1"></i> Tampil' => 1,
                                                        '<i class="fas fa-minus-square mr-1"></i> Sembuyikan' => 0,
                        ...
                        <script>
                            ...
                            $("#dynamic-ar").click(function() {
                                ...
                                $("#dynamicAddRemove").append(
                                    `<tr>
                            <td>
                                <input type="text" name="timeline_name[]" placeholder="Enter subject" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" required/>
                            </td>
                            
                            <td>
                                <input type="datetime-local" name="timeline_time[]" placeholder="Enter subject" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" required/>
                            </td>
                            <td>
                                <input type="datetime-local" name="timeline_time_end[]" placeholder="Enter subject" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" required/>
                            </td>
                            <td>
                                <button type="button" class="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded-md py-1 px-3 leading-normal no-underline text-red-600 border-red-600 hover:bg-red-600 hover:text-white bg-white hover:bg-red-700 remove-input-field">Delete</button>
                            </td>
                        </tr>`
                                );
                            });
                            $("#dynamic-ar-image").click(function() {
                                ++i;
                                $("#dynamicAddRemoveImage").append(
                                    `<tr>
                                    <td>
                                        <div class="custom-file">
                                            <input type="file" name="dokumentasi[]" id="dokumentasi"
                                                onchange="openFile(event, '#img-logo')"
                                                required
                                ...
                                */
                                @if ($errors->first('updateUser_user_id'))
                                    $('#m-edit-proker-user')
                                        .find('form')
                                        .attr('action',
                                            `{{ route('dashboard.admin.prokers.update.user', $errors->first('updateUser_user_id')) }}`
                                        );
