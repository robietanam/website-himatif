...
<div class="mb-4">
    ...
    @foreach ($inputDatas as $datakey => $datavalue)
        <div class="custom-control custom-radio">
            <input type="radio" name="{{ $inputName }}" value="{{ $datavalue }}"
                ...
                @if (isset($inputIsRequired) && $inputIsRequired === true) required @endif @if (isset($inputIsDisabled) && $inputIsDisabled === true) disabled @endif
                class="custom-control-input @error($inputName) bg-red-700 @enderror">
            <label class="custom-control-label font-bold"
                for="input-radio-{{ $inputName }}-{{ $datavalue }}">{!! $datakey !!}</label>
        </div>
    @endforeach
    @isset($inputHelp)
        <div class="text-12 text-gray-700">{{ $inputHelp }}</div>
    @endisset
